...
const BAddSelectedVariant: React.FC<SelectedVariantProops> = ({
  ...
  return (
    <Button
      ...
      className={
        isButtonDisabled
          ? "bg-red-primary border-2 border-red-primary text-text-action rounded-lg"
    ...
    >
      {isButtonDisabled ? (
        <Check className="w-5 h-5 font-extrabold"></Check>
      ) : (
        <Plus className="w-5 h-5 font-extrabold"></Plus>
      )}
    </Button>
  );
};
