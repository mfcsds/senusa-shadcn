  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={"ghost"} className="ml-2 text-blue-primary hover:text-text-primary hover:bg-foreground">
      ...
      <PopoverContent className="w-[600px] max-w-full p-6">
        <div className="grid gap-4">
          <div className="space-y-2">
            <h4 className="font-medium leading-none text-text-primary">
          ...
          <div className="grid grid-cols-5 gap-4 max-h-[400px] overflow-y-auto border p-4 rounded-lg ">
            {geneList.map((gene, index) => (
              <div
                key={index}
                className="text-sm bg-background p-2 rounded-md shadow-sm hover:bg-accent text-text-primary"
              >
                {gene}
              </div>
            ))}
