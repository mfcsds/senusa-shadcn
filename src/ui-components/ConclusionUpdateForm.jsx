...
import { getConclusion } from "../graphql/queries";
import { updateConclusion } from "../graphql/mutations";
const client = generateClient();
export default function ConclusionUpdateForm(props) {
  const {
    id: idProp,
    conclusion: conclusionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [conclusionRecord, setConclusionRecord] =
    React.useState(conclusionModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getConclusion.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getConclusion
        : conclusionModelProp;
      setConclusionRecord(record);
    };
    queryData();
  }, [idProp, conclusionModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateConclusion.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "ConclusionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Text"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || conclusionModelProp)}
