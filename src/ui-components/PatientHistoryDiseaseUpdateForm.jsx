...
import { getPatientHistoryDisease } from "../graphql/queries";
import { updatePatientHistoryDisease } from "../graphql/mutations";
const client = generateClient();
export default function PatientHistoryDiseaseUpdateForm(props) {
  const {
    id: idProp,
    patientHistoryDisease: patientHistoryDiseaseModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = patientHistoryDiseaseRecord
      ? { ...initialValues, ...patientHistoryDiseaseRecord }
      : initialValues;
  ...
  const [patientHistoryDiseaseRecord, setPatientHistoryDiseaseRecord] =
    React.useState(patientHistoryDiseaseModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPatientHistoryDisease.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPatientHistoryDisease
        : patientHistoryDiseaseModelProp;
      setPatientHistoryDiseaseRecord(record);
    };
    queryData();
  }, [idProp, patientHistoryDiseaseModelProp]);
  React.useEffect(resetStateValues, [patientHistoryDiseaseRecord]);
  const validations = {
    id_patient: [],
    hpo_code: [],
    hpo_desc: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePatientHistoryDisease.replaceAll("__typename", ""),
            variables: {
              input: {
                id: patientHistoryDiseaseRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PatientHistoryDiseaseUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id patient"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || patientHistoryDiseaseModelProp)}
