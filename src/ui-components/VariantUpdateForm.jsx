...
import { getVariant } from "../graphql/queries";
import { updateVariant } from "../graphql/mutations";
const client = generateClient();
export default function VariantUpdateForm(props) {
  const {
    id: idProp,
    variant: variantModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getVariant.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getVariant
        : variantModelProp;
      setVariantRecord(record);
    };
    queryData();
  }, [idProp, variantModelProp]);
  React.useEffect(resetStateValues, [variantRecord]);
  const validations = {
    chrom: [],
    pos: [],
    ref: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateVariant.replaceAll("__typename", ""),
            variables: {
              input: {
                id: variantRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "VariantUpdateForm")}
      {...rest}
    >
      <TextField
        label="Chrom"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || variantModelProp)}
